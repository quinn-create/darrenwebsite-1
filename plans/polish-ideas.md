...
7. **Light theme option** (Microsoft, GitHub). Some older visitors read dark pages less easily. **Built 25 Sep 2026** (`plans/light-theme-plan.md`): a sun button in the header switches to light colours and is remembered; the site still opens dark. Off switch: `LIGHT_THEME`.
8. **Speed polish** (Amazon's focus on speed). **Built 25 Sep 2026** (`plans/speed-check-plan.md`): no more text jumping while the font loads, 22 KB less JavaScript on every page, and a repeatable `npm run speed` test with budgets.
