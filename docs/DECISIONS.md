...
| D9 | Visitor statistics tool | Plausible (cookie-free, no banner) | **Answered** (C5) | The built cookie banner stays (GA4, Google Ads, Meta Pixel, off until IDs are set), with **no tag on the Contact/form page**. Darren's written OK naming each tag is still needed before any ID is entered. | 26 Sep 2026 | `answers-2026-09-26.md` |
...
| C8 | Hosting: the plan and D17 say the firm's Vercel account | Moving to Cloudflare (free Workers plan), where the domain already is | Quinn, 26 Sep 2026 (`answers-2026-09-26.md`) | **Decided.** Conversion to be built and tested; the free plan's per-visit computing limit may push the form to Cloudflare's $5/month plan |
